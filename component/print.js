...
const fs = require("fs");
const printingItem = async (data, orderData, invoice) => {
  console.log("printing ---------------", data);
  console.log("printing2 ---------------", orderData.dataValues);
  ...
  const timeString = `${hours12}:${minutes
    .toString()
    .padStart(2, "0")} ${ampm}`;
  canvasHeight = 800 + data.length * 50;
  ...
  const lineHeight = 0;
  const nameH = lineHeight + 170;
  const addH = nameH + 30;
  const twoaddH = addH + 30;
  const phoneH = twoaddH + 35;
  ...
  const secondLineH = itemHeight + data.length * 40;
  const subTotalH = secondLineH + 30;
  let discountH = subTotalH + 35;
  let grandTotalH = discountH + 35;
  let thirdLineH = grandTotalH + 35;
  let payamountH = thirdLineH + 35;
  let cashBackH = payamountH + 35;
  let thankH = cashBackH + 60;
  let seeH = thankH + 30;
  ctx.font = "28px Comic Sans Ms";
  ctx.fillText(`MULA ART GALLERY`, canvas.width / 2, nameH);
  ctx.font = "20px Comic Sans Ms";
  ctx.fillText(`E 1-12,The Secretariat Yangon,`, canvas.width / 2, addH);
  ctx.fillText(
  ...
  ctx.textAlign = "center";
  ctx.font = "24px Pyidaungsu";
  ctx.fillText(`Invoice No : ${invoice}`, canvas.width / 2, tableH);
  ...
  data.forEach((productItem, index) => {
    ctx.font = "24px Pyidaungsu";
    let yPos = itemHeight + index * 40;
    ctx.textAlign = "start";
    ctx.fillText(productItem.name, 20, yPos);
    ctx.textAlign = "right";
  ...
  ctx.textAlign = "right";
  ctx.fillText(`${orderData.grandtotal + orderData.discount}`, 550, subTotalH);

  //discount charge
  ctx.textAlign = "center";
  ctx.fillText(`Discount  : `, 385, discountH);
  ctx.textAlign = "right";
  ctx.fillText(`${orderData.discount}`, 550, discountH);

  //service charge
  ctx.textAlign = "center";
  ctx.fillText(`Grand Total  : `, 370, grandTotalH);
  ctx.textAlign = "right";
  ctx.fillText(`${orderData.grandtotal}`, 550, grandTotalH);
  ...
  //cashback
  ctx.textAlign = "center";
  ctx.fillText(`PayAmount  : `, 373, payamountH);
  ctx.textAlign = "right";
  ctx.fillText(`${orderData.payamount}`, 550, payamountH);
  ctx.font = "24px Comic Sans Ms";
  //discount
  ctx.textAlign = "center";
  ctx.fillText(`CashBack  : `, 378, cashBackH);
  ctx.textAlign = "right";
  ctx.fillText(`${orderData.payback}`, 550, cashBackH);
  ctx.font = "24px Comic Sans Ms";
  //thank you
  ctx.textAlign = "center";
  ctx.fillText(
    "Thank You For Enjoying Our Artworks.",
    canvas.width / 2,
    thankH
  );
  ctx.textAlign = "center";
  ctx.fillText("See You  Again Soon!", canvas.width / 2, seeH);
  const filename = "orderImages/receipt.png";
  const buffer = canvas.toBuffer("image/png");
  fs.writeFileSync(filename, buffer);
  // await networkPrinterPrint(buffer, "192.168.0.106");
};
